...
export function generateCode(node: ASTNode): string {
  ...
  if (node.type === "Directive") {
    ...
    if (node.directive === "for") {
      const match = /(\w+)\s+of\s+([^;]+);\s*key:\s*(.+)/.exec(
        node.condition ?? "",
      );
      if (match) {
...
export function generateCodeNodes(nodes: ASTNode[]): string {
  const result: string[] = [];
  let i = 0;
  ...
  while (i < nodes.length) {
    ...
    if (node.type === "Directive" && node.directive === "for") {
      ...
      if (match) {
        ...
        result.push(
          `h(For, { each: () => ${array.trim()}, key: (${variable}) => ${key.trim()}, render: (${variable}) => ${renderCode}${emptyCode} })`,
        );
        if (hasEmpty) i++;
      }
      ...
      result.push(
        `h(If, { when: () => ${node.condition}, then: () => ${thenCode}${elseCode} })`,
      );
      if (hasElse) i++;
    } else {
