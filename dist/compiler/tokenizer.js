export function tokenize(template) {
    ...
    while (pos < template.length) {
        ...
        if (template.startsWith("<!--", pos)) {
            ...
            if (end === -1)
                throw new Error(`[Moonie] Comentario sin cerrar en línea ${line}. ¿Se te olvidó "-->"?`);
        ...
        if (template[pos] === "{" && template[pos + 1] === "{") {
            const end = template.indexOf("}}", pos);
            if (end === -1)
                throw new Error(`[Moonie] Interpolación sin cerrar en línea ${line}. ¿Se te olvidó "}}"?`);
        ...
        if (template[pos] === "{") {
            ...
            if (depth > 0)
                throw new Error(`[Moonie] Bloque sin cerrar que comenzó en línea ${startLine}. ¿Se te olvidó "}"?`);
